...
from resnet_geom_tof_pretrained import *
...
def getPositionPrediction(out,offset=2):
    out_pos            = out[:,(offset+0):(offset+3)]
    out_logSigmaPosSqr = out[:,(offset+3):(offset+5)]
...
execfile(os.path.dirname(__file__)+"/forward_releposdir_longtrans_tof.py")
execfile(os.path.dirname(__file__)+"/inference_releposdir_tof.py")
...
# first load the pretrained longtrans network for doing the "simple" position estimation
simple_blob=BLOB()
simple_blob.net       = ResNetGeom18(11).cuda() # E,Eres,pos[3],posres[2],dir[3],dirres
# just the training weights is ~700MiB
simple_blob.optimizer = torch.optim.Adam(simple_blob.net.parameters()) # use Adam optimizer algorithm
simple_blob.prefix    = '20200827-05-IWCD-SmallResNetGeom-timeCmplxNCoord-relEposdir-01-relEposdir-n3d-longtrans'
simple_blob.epoch     = 0.
simple_blob.iteration = 0
simple_blob.data      = None # data for training/analysis
simple_blob.label     = None # label for training/analysis

simple_blob.prefix += '-20200922-041914'
restore_state(simple_blob, 909318)


blob=BLOB()
blob.net       = ResNetGeomTofPretrained18(simple_blob.net, 11).cuda() # E,Eres,pos[3],posres[2],dir[3],dirres
blob.optimizer = torch.optim.Adam(blob.net.parameters()) # use Adam optimizer algorithm
blob.prefix    = '20200926-02-IWCD-ResNetGeomTOF-timeCmplxNCoord-relEposdir-01-relEposdir-n3d-LT-pretrained'
blob.epoch     = 0.
blob.iteration = 0
...
if sys.argv[1] == "reco":
    # blob.prefix += '-20200928-103820'
    # restore_state(blob, 107938)
    blob.prefix += '-20200928-184300'
    restore_state(blob, 97938)
    execfile(os.path.dirname(__file__)+'/process_events_tof.py')
