...
class BLOB:
    pass
blob=BLOB()
blob.net       = ResNetGeomTof18(11).cuda() # E,Eres,pos[3],posres[2],dir[3],dirres
# just the training weights is ~700MiB
blob.optimizer = torch.optim.Adam(blob.net.parameters()) # use Adam optimizer algorithm
blob.prefix    = '20200926-07-IWCD-ResNetGeomTOF-timeCmplxNCoord-relEposdir-01-relEposdir-n3d-longtrans'
blob.epoch     = 0.
blob.iteration = 0
...
if sys.argv[1] == "reco":
    blob.prefix += '-20200927-090722'
    # restore_state(blob, 8000)
    restore_state(blob, 199876)
