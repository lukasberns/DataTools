...
from common import *
from resnet_geom import BasicGeomBlock
...
class ResNetMHGeom(nn.Module):
    def __init__(self, block, num_blocks, head_num_classes=[3], overall_in_planes=8, overall_ch=64, initial_stride=[2,2], use_layer0=False, dropout=0., use_MLP_BN=False):
        ...
        head_num_classes: an array with an integer for each head
                          ...
                          for the given head. The final output will
                          simply concatenate the outputs from each head,
        ...
        # and use_layer0 = True
        super(ResNetMHGeom, self).__init__()
        self.pad = 1
    ...
    def forward(self, x):
        ...
        return torch.cat([head(out) for head in self.heads],1)
