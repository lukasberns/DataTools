...
def getNllPosition(out_pos, out_logSigmaPosSqr, true_pos, true_dir):
    ...
    lsprT = lsprL - 2.*np.log(5.) # force the transverse resolution 5 times smaller than the longitudinal
    out_logSigmaPosSqr = lsprL # remove the second degree of freedom since it will be unbounded inthe torch.sum(out_logSigmaPosSqr,1) term
...
blob.prefix    = '20210118-03-IWCD-SmallResNetGeom-timeCmplxNCoord-relEposdir-01-relEposdir-n3d-longtransX'
blob.epoch     = 0.
blob.iteration = 0
...
if sys.argv[1] == "reco":
    blob.prefix += '-20210122-104731'
    restore_state(blob, 359752)
