...
def forward(blob,train=True):
    ...
    with torch.set_grad_enabled(train):
        ...
        out = blob.net(data)
        simple = out[:,0:5] # this is a small output for a rough pos estimate for tof subtraction
        out    = out[:,5:]

        simple_pos,simple_posres = getPositionPrediction(simple, offset=0)
        ...
        true_pos = torch.as_tensor(blob.position/1000.).type(torch.FloatTensor).cuda()
        ...
        nll_simple_position = getNllPosition(simple_pos, simple_posres, true_pos, true_dir)
        nll_position        = getNllPosition(out_pos,    out_posres,    true_pos, true_dir)
        ...
        loss_simple_position = torch.sum(nll_simple_position)
        loss_position = torch.sum(nll_position)
        loss_direction = torch.sum(nll_direction)
        loss = loss_energy + loss_simple_position + loss_position + loss_direction
        blob.loss = loss
        ...
        return {'pred_Eabovethres' : out_Eabovethres.cpu().detach().numpy(),
                ...
                'pred_simple_position'    : simple_pos   .cpu().detach().numpy(),
                'pred_simple_positionres' : simple_posres.cpu().detach().numpy(),
                ...
                'loss_simple_position' : loss_simple_position.cpu().detach().item(),
...
resKeysToLog = ['loss_energy','loss_simple_position','loss_position','loss_direction','loss','mean_chi2_E','mean_chi2_pos','mean_chi2_dir','mean_Eres','mean_posres','mean_dirres']
