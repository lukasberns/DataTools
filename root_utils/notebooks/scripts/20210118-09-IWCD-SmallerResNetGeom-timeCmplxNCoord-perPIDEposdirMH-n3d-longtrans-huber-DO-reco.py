...
class BLOB:
    pass
blob=BLOB()
blob.net       = ResNetMHGeom18(npid*np.array([1, 1+1, 3+2, 3+1]), dropout=0.3).cuda() # {pid[3]},and for each pid {E,Eres},{pos[3],posres[2]},{dir[3],dirres}
# just the training weights is ~700MiB
blob.optimizer = torch.optim.Adam(blob.net.parameters()) # use Adam optimizer algorithm
blob.prefix    = '20210118-09-IWCD-SmallResNetGeom-timeCmplxNCoord-perPIDEposdirMH-01-n3d-longtrans-huber-DO'
blob.epoch     = 0.
blob.iteration = 0
